...
router.get('/:username', async (req, res) => {
    ...
    try{
        ...
        if(username == null || username == undefined){
            ...
            const user = await User.findOne({ username: username }, ['username','lezioni',"professore"]).exec();
            if(user == null){
                //utente non trovato nel database, viene ritornato un errore all'utente
                message = "Utente non presente nel database";
                res.status(400).json({ message: message });
            }else if(!user.professore){
                //Se l'utente non è un professore ritorna errore
                message = "Erore l'utente deve essere un professore";
                res.status(400).json({ message: message });
            }
            else{
                ...
                let elenco_lezioni = []
                //mantieni solo le lezioni non prenotate
